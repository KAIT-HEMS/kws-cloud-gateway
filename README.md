...
<small style="color:red;">※ 機器エミュレータをデバイスとして使用する場合の注意事項：機器エミュレータ起動後にデバイスを追加や削除しても、クラウド Gateway は追加・削除した機器を自動では認識しません。追加・削除したデバイスをデバイス管理に反映するには、デバイス管理画面でデバイス情報初期化をクリックするか、クラウド Gateway を再起動してください。</small>
