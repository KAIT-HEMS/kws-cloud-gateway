...
class HttpAdmin {
  ...
  * ---------------------------------------------------------------- */
  ...
  async _showAccessCodeForm(req, res) {
    ...
    // ページを表示
    this._render(req, res, csrf_name);
  }
  ...
  async _showApiTokenForm(req, res) {
    ...
    // ページを表示
    this._render(req, res, csrf_name, {
      api_token_json_data: this._conf.api_token_json_data
    });
  }
  ...
  async _setApiTokenForm(req, res) {
    ...
    // ポストデータを取得
    const api_token_json = req.body.api_token_json;

    // セッションにデータを保存
    req.session.locals.form = {
      api_token_json: api_token_json
    };
    ...
    // ポストデータをチェック
    if (!api_token_json) {
      respondError('API トークン JSON を指定してください。', ['api_token_json']);
      return;
    }

    let data = null;
    try {
      data = JSON.parse(api_token_json);
    } catch (e) {
      respondError('アップロードされたファイルは JSON ではありません。', ['api_token_json']);
      return;
    }

    if (typeof (data) !== 'object' || !data.name || !data.gateway_token || !data.remote_token) {
      respondError('アップロードされた JSON に必要な情報が含まれていません。', ['api_token_json']);
      return;
    }

    if (typeof (data.name) !== 'string' || data.name.length > 255) {
      respondError('name の値が不正です。', ['api_token_json']);
      return;
    }

    if ('org' in data) {
      if (typeof (data.org) !== 'string' || data.org.length > 255) {
        respondError('name の値が不正です。', ['api_token_json']);
        return;
      }
    }

    if (typeof (data.gateway_token) !== 'string' || data.gateway_token.length > 255) {
      respondError('gateway_token の値が不正です。', ['api_token_json']);
      return;
    }

    if (typeof (data.remote_token) !== 'string' || data.remote_token.length > 255) {
      respondError('remote_token の値が不正です。', ['api_token_json']);
      return;
    }

    let data_tmp = JSON.parse(JSON.stringify(data));
    delete data_tmp.name;
    delete data_tmp.org;
    delete data_tmp.gateway_token;
    delete data_tmp.remote_token;
    if (Object.keys(data_tmp).length > 0) {
      respondError('アップロードされた JSON に未知のプロパティが含まれています。', ['api_token_json']);
      return;
    }
    ...
    try {
      const udata = {
        api_token: data.gateway_token,
        api_token_json_data: data
      };
      await this._uconf_manager.update(udata);
      this._onuconfupdated(udata);
