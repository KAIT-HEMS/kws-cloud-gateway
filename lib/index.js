  ...
  * ---------------------------------------------------------------- */
  ...
  _initWebSocket() {
    ...
    if (!this._conf.api_token) {
      this._debug.log('Gateway 用 API トークンが設定されていません。ブラウザーで http://localhost:8880/admin/index にアクセスし、API トークンを設定してください。');
      return;
    }
    ...
    this._ws.on('connectFailed', async (error) => {
      ...
      const msg = 'インターネット接続を確認してください。なおクラウド Gateway は Proxy には対応していません。インターネット接続が正常にもかかわらず WebSocket 接続できない場合は、Gateway 用トークンが有効であるかをサーバー管理者に確認ください。';
