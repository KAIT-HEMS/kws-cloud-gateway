...
if(ver_m) {
  ...
  if(major_ver < 12) {
    // v12 未満なら終了
    let msg = 'node.js のバージョンが v12 未満です。hems-controller は動作しないため終了します。(現在の node.js のバージョン: ' + node_ver + ')';
    console.log('\n');
...
// HEMS コントローラー起動
const mIndex = require('./lib/index.js');
let hemscontroller = new mIndex({ base_dir: __dirname, version: version });

hemscontroller.start(options).then(() => {
  // 何もしない
}).catch((error) => {
  console.error(error);
  process.exit();
